...
export default function MesasBarClient({ categories, coverShow, staff, storeName, machines, endereco, cnpj, tel, cupomRodape, onSaleClosed, canClose = true, selfWaiterId = null }: {
  ...
  // couvert = cover/pessoa × pessoas. Usa o snapshot (cover) se já aplicado; senão calcula do show ×
  // pessoas (assim o valor aparece mesmo antes do 1º ajuste). coverOn é o toggle (igual os 10%).
  const coverFull = cover > 0 ? cover : (coverShow?.coverCents ?? 0) * people;
  const coverCharged = coverOn ? coverFull : 0;
  ...
  return (
    <>
      ...
      <div className="mb-4 flex items-center justify-between gap-2">
        {canClose ? <QzStatus /> : <span />}
        {canClose && <div className="flex items-center gap-2">
        {addOpen ? (
          <>
            ...
            {view === "pick" ? (
              ...
              <>
                ...
                <div className="mt-3 space-y-1 text-sm">
                  ...
                  {coverShow ? (
                    // Couvert = SEMPRE um checkbox (igual os 10%): marcado cobra, desmarca pra remover se o
                    // cliente recusar. Ao lado, o ajustador de pessoas (cover/pessoa × pessoas).
                    <div className="flex items-center justify-between text-[var(--text-muted)]">
                      ...
                      <span className="tabular-nums">{coverOn ? brl(coverFull) : <span className="text-[var(--text-faded)] line-through">{brl(coverFull)}</span>}</span>
                    </div>
                  ) : (cover > 0 && (

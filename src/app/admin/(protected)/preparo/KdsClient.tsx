...
type KdsItem = { name: string; size_label: string | null; qty: number; mods: { name: string; price_cents: number }[] | null; note?: string | null; no_prep?: boolean };
...
export default function KdsClient({ stations, loja, kitchenScreen = true }: { stations: string[]; loja: string; kitchenScreen?: boolean }) {
  ...
  function renderCard(o: KdsOrder, showAdvance: boolean) {
    const items = o.items.filter((i) => !i.no_prep);
        ...
        <ul className="space-y-1">
          {items.map((it, i) => (
            <li key={i} className="text-sm">
              ...
              {it.mods && it.mods.length > 0 && <div className="pl-6 text-xs font-bold text-amber-700">{it.mods.map((m, j) => <div key={j}>+ {m.name}</div>)}</div>}
  ...
  return (
    <div>
      ...
      {kitchenScreen ? (
        /* cozinha COM tela: kanban com status (bump) */
        ...
        /* cozinha SÓ TICKET: quadro de LEITURA — o que foi pedido e ainda não saiu (sem clicar status) */
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {orders.every((o) => o.items.every((i) => i.no_prep)) && <p className="col-span-full py-10 text-center text-sm text-zinc-300">Nada pendente no preparo.</p>}
          {orders.map((o) => renderCard(o, false))}
        </div>
      )}
    </div>
  );

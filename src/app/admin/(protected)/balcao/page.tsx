...
export default async function BalcaoPage() {
  ...
  return (
    <>
      ...
      <BalcaoClient categories={categories} storeName={store.name} machines={machines} endereco={store.endereco} cnpj={store.cnpj} tel={store.whatsapp} cupomRodape={store.cupomRodape} loyaltyEnabled={!!cfg?.loyalty_enabled} offlineEnabled={!!cfg?.offline_enabled} />
    </>
  );
}

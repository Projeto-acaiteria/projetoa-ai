...
// Cache de 60s por loja (era force-dynamic): o site do Starteq lê isso a CADA visita. Cacheado,
// vira ~1 leitura do banco a cada 60s por loja, independente de visitas — corta egress. O estoque
// (stock) fica até 60s desatualizado: ok pro perfil AT (baixa frequência, sem promoção-relâmpago).
const buildCatalog = (slug: string) =>
  unstable_cache(
    async () => {
      ...
      return { slug, store: s.name, count: products.length, pixDiscountPercent: pixPct, products };
    },
    ["loja-catalogo", slug],
    { revalidate: 60, tags: [`catalogo-${slug}`] },
  )();

export async function GET(_req: Request, { params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const payload = await buildCatalog(slug);
  if (!payload) return NextResponse.json({ error: "loja não encontrada" }, { status: 404, headers: CORS });

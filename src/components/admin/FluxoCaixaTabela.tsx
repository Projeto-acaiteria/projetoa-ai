...
export type Venda = { display: string; date: string; mode: string; paymentMethod: string | null; grossCents: number; cardFeeCents: number; netCents: number; customerName: string; items?: { name: string; qty: number; paidCents: number }[]; discountCents?: number; payments?: { method: string; amountCents: number }[] | null };
...
const MODO_LABEL: Record<string, string> = { balcao: "Balcão", retirada: "Retirada", entrega: "Entrega", mesa: "Mesa" };

function DrillModal({ data, onClose }: { data: { title: string; vendas?: Venda[]; despesas?: Despesa[] }; onClose: () => void }) {
  const hm = (d: string) => { try { return new Date(d).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit", timeZone: "America/Sao_Paulo" }); } catch { return ""; } };
  const cliente = (n?: string) => (n && !["Balcão", "Cliente", ""].includes(n.trim()) ? n.trim() : null);
  const metodoLabel = (v: Venda) =>
    v.payments && v.payments.length > 1
      ? v.payments.map((p) => `${PAY_LABEL[p.method] ?? p.method} ${brl(p.amountCents)}`).join(" + ")
      : v.paymentMethod ? PAY_LABEL[v.paymentMethod] : MODO_LABEL[v.mode] ?? v.mode;

  const vendas = data.vendas ?? [];
  const despesas = data.despesas ?? [];
  const n = vendas.length + despesas.length;
  const grossV = vendas.reduce((s, v) => s + v.grossCents, 0);
  const feeV = vendas.reduce((s, v) => s + v.cardFeeCents, 0);
  const netV = vendas.reduce((s, v) => s + v.netCents, 0);
  const totalD = despesas.reduce((s, e) => s + e.amountCents, 0);
  const byMethod: Record<string, number> = {};
  for (const v of vendas) { const k = v.paymentMethod ?? "outro"; byMethod[k] = (byMethod[k] || 0) + v.grossCents; }
  const methodOrder = ["dinheiro", "pix", "debito", "credito"].filter((k) => byMethod[k]);

  return (
    <div className="fixed inset-0 z-50">
      <div className="absolute inset-0 bg-black/40" onClick={onClose} />
      <div className="absolute inset-x-0 bottom-0 mx-auto flex max-h-[88vh] max-w-lg flex-col rounded-t-3xl bg-bg-elevated shadow-[var(--shadow-pop)] sm:inset-x-auto sm:left-1/2 sm:top-1/2 sm:bottom-auto sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rounded-3xl">
        <div className="border-b border-line p-5">
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-[var(--text-muted)]">{n} movimentação{n === 1 ? "" : "s"}</p>
            </div>
            <div className="text-right">
              <div className={`text-xl font-extrabold ${despesas.length ? "text-[var(--red-no)]" : "text-lime"}`}>{despesas.length ? `− ${brl(totalD)}` : brl(netV)}</div>
              {!despesas.length && feeV > 0 && <div className="text-[11px] text-[var(--text-faded)]">bruto {brl(grossV)} · taxa {brl(feeV)}</div>}
            </div>
          </div>
          {/* resumo por forma de pagamento (conferência rápida) */}
          {methodOrder.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-1.5">
              {methodOrder.map((k) => (
                <span key={k} className="rounded-full bg-bg-surface-2 px-2.5 py-1 text-[11px] font-bold text-ink-2">
                  {PAY_LABEL[k]} <span className="text-ink">{brl(byMethod[k])}</span>
                </span>
              ))}
            </div>
          )}
        ...
        <div className="min-h-0 flex-1 space-y-2 overflow-y-auto p-3">
          {vendas.map((v) => {
            const cli = cliente(v.customerName);
            return (
              <div key={v.display} className="rounded-xl border border-line p-3">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <div className="text-sm font-bold text-ink">{v.display} <span className="text-[var(--text-muted)]">· {MODO_LABEL[v.mode] ?? v.mode}</span></div>
                    <div className="text-[11px] text-[var(--text-muted)]">{hm(v.date)}{cli ? ` · ${cli}` : ""}</div>
                  </div>
                  <div className="shrink-0 text-right">
                    <div className="text-sm font-extrabold text-lime tabular-nums">{brl(v.netCents)}</div>
                    {v.cardFeeCents > 0 && <div className="text-[10px] text-[var(--text-faded)]">bruto {brl(v.grossCents)} − taxa {brl(v.cardFeeCents)}</div>}
                  </div>
                </div>
                {v.items && v.items.length > 0 && (
                  <ul className="mt-2 space-y-0.5 border-t border-line pt-2">
                    {v.items.map((it, i) => (
                      <li key={i} className="flex justify-between gap-2 text-xs text-ink-2">
                        <span className="min-w-0 truncate">{it.qty > 1 ? `${it.qty}× ` : ""}{it.name}</span>
                        <span className="shrink-0 tabular-nums text-[var(--text-muted)]">{it.paidCents > 0 ? brl(it.paidCents) : "grátis"}</span>
                      </li>
                    ))}
                  </ul>
                )}
                <div className="mt-1.5 flex flex-wrap items-center gap-x-2 text-[11px] text-[var(--text-muted)]">
                  <span className="font-semibold text-ink-2">{metodoLabel(v)}</span>
                  {v.discountCents ? <span>· desconto {brl(v.discountCents)}</span> : null}
                </div>
              </div>
            );
          })}
          {despesas.map((e) => (
            <div key={e.id} className="flex items-center justify-between gap-2 rounded-xl border border-line p-3">
              <div className="min-w-0">
                <div className="truncate text-sm font-bold text-ink">{e.description}</div>
                <div className="text-[11px] text-[var(--text-muted)]">{e.date.split("-").reverse().join("/")} · {CAT_LABEL[e.category]}</div>
              </div>
              <div className="shrink-0 text-sm font-extrabold text-[var(--red-no)] tabular-nums">− {brl(e.amountCents)}</div>
            </div>
          ))}
          {n === 0 && <p className="py-6 text-center text-sm text-[var(--text-faded)]">Sem movimentações no período.</p>}
        </div>
